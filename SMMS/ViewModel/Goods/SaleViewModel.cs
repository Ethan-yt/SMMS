...
namespace SMMS.ViewModel.Goods
{
    public class SaleViewModel : ViewModelBase
    {
        ...
        public RelayCommand SaleCommand
        {
            get
            {
                return new RelayCommand(() =>
                {
                    ...
                    try
                    {
                        intNum = int.Parse(GoodsNum);
                    }
                    catch
                    {
                        ModernDialog.ShowMessage("销售失败！请检查数量设置是否正确。", "失败", System.Windows.MessageBoxButton.OK);
                        return;
                    }
                    ...
                    foreach (var goods in selectedGoods)
                    {
                        ...
                        try
                        {
                            DBHelper.updateGoods(goods.GID, "NUM", (goods.NUM - intNum).ToString(),false,true,intNum);
                        }
                        catch
                        {
                            ModernDialog.ShowMessage("销售失败！库存不足。\r\n\r\n货号：" + goods.GID+ "\t剩余库存："+goods.NUM, "失败", System.Windows.MessageBoxButton.OK);
                            t.Rollback();
                            return;
                        }
                    }

                    ModernDialog.ShowMessage("销售成功！", "成功", System.Windows.MessageBoxButton.OK);
                    t.Commit();
